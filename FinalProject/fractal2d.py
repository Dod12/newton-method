...
class Fractal2D(object):
    ...
    def newton_zeros(
        ...
        if self.jacobian is not None:
            ...
            x_n = self._newton_helper_estimate_jacobian(self.function, x_0, n_iter, h, loop_tolerance, upper_lim, simplified)
    ...
    def newton_index(self, x0: NDArray[Real], tol: float = 1e-9, up: float =1.e+8, simple: bool =False, **kwargs) -> int:
        """
    ...
    def _newton_helper(function: Callable, jacobian: Callable, x_0: NDArray[Real], n_iter: int = 10000,
                       loop_tolerance: float = np.finfo(np.float64).eps, upper_lim: float=1.e+8, simplified: bool =False) -> NDArray[np.float64]:
        """
