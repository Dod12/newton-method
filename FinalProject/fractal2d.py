...
def _newton_helper(function: Callable, jacobian: Callable, x_0: NDArray[Real], n_iter: int = 10000,
    ...
    while not np.linalg.norm(function(x_n)) < loop_tolerance and iterations < n_iter:
        if not (np.any(np.isfinite(x_n)) and np.any(np.isfinite(jacobian(x_n))) and np.any(np.isfinite(function(x_n)))) or np.linalg.det(jacobian(x_n)) == 0:
            break
...
class Fractal2D(object):
    ...
    def newton_zeros(
        ...
        if self.jacobian is not None:
            x_n = _newton_helper(self.function, self.jacobian, x_0, n_iter, loop_tolerance)
        else:
            x_n = _newton_helper_estimate_jacobian(self.function, x_0, n_iter, h, loop_tolerance)
